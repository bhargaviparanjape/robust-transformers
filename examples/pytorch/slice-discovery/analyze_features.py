...
def create_features(training_args, trainer, dataloader, model, config, split, is_pretraining=False):
    ...
    # Dump results for deviation measuremenst.
    # For vocab, find tokens whose adjusted sim score is most or least with pretrained representations, embeddings and  
...
def main():
    ...
    if data_args.create_features:
        ...
        # train_dataloader = trainer.get_train_dataloader()
        # create_features(training_args, trainer, train_dataloader, pretrained_model, config, "train", is_pretraining=True)

        eval_dataloader = trainer.get_eval_dataloader(eval_dataset)
        finetuned_hidden, lengths = create_features(training_args, trainer, eval_dataloader, model, config, split="dev")
        ...
        eval_dataloader = trainer.get_eval_dataloader(eval_dataset)
        pretrained_hidden, _ = create_features(training_args, trainer, eval_dataloader, pretrained_model, config, "dev", is_pretraining=True)

        # Analyse deviations
        # For each token, find cosine sim. 
        # Output unique tokens for which cosine sim is low or high
        token_ids = []
        cosines = []
        for i, (x, x_pre) in enumerate(zip(finetuned_hidden, pretrained_hidden)):
            x_emb = x[0, :lengths[i], :]
            x_emb_pre = x_pre[0, :lengths[i], :]
            import pdb; pdb.set_trace()

        # tokenizer._convert_id_to_token(6179)
